...
export const signIn = async ({ email, password }: signInProps) => {
  try {
    ...
    const user = await getUserInfo({userId : session.userId}); //fetch from realdatabase not from session
...
export const createBankAccount = async ({
  ...
  fundingSourceUrl,
  sharableId, 
}: createBankAccountProps) => {  sharableId
  try {
    const { database } = await createAdminClient();
...
export const getBankByAccountId = async ({accountId}: getBankByAccountIdProps) => {
  try {
    ...
    if (bank.total !== 1) return null; // No bank found with the given accountId
    return parseStringify(bank.documents[0]);
  } catch (error) {
    console.log("An error occurred while getting the banks:", error);
  }
